...
public class WorldGenBakuTree extends WorldGenAbstractTree {

    public WorldGenBakuTree(boolean bool)
    {
        super(bool);
    }
            ...
            else
            {
                ...
                if (isSoil && y < 256 - l - 1)
                {
                    ...
                    for (j2 = 0; j2 < l; ++j2)
                    {
                        ...
                        {
                            this.setBlockAndNotifyAdequately(world, k3, k2, l1, ZenScape.blockZenLog, 2);
                            i2 = k2;
                        }
                    }
                    ...
                    for (j2 = -1; j2 <= 1; ++j2)
                    {
                        for (k2 = -1; k2 <= 1; ++k2)
                        {
                            this.generateLeaves(world, k3 + j2, i2 + 1, l1 + k2);
                        }
                    }

                    this.generateLeaves(world, k3 + 2, i2 + 1, l1);
                    this.generateLeaves(world, k3 - 2, i2 + 1, l1);
                    this.generateLeaves(world, k3, i2 + 1, l1 + 2);
                    this.generateLeaves(world, k3, i2 + 1, l1 - 2);
                    ...
                    for (j2 = -3; j2 <= 3; ++j2)
                    {
                        for (k2 = -3; k2 <= 3; ++k2)
                        {
                            ...
                            {
                                this.generateLeaves(world, k3 + j2, i2, l1 + k2);
                            }
                        }
                    }
                    ...
                    if (j2 != j3)
                    {
                        ...
                        for (l2 = k2; l2 < l && l3 > 0; --l3)
                        {
                            if (l2 >= 1)
                            {
                                ...
                                {
                                    this.setBlockAndNotifyAdequately(world, k3, i3, l1, ZenScape.blockZenLog, 2);
                                    i2 = i3;
                                }
                            }
                        ...
                        if (i2 > 0)
                        {
                            for (l2 = -1; l2 <= 1; ++l2)
                            {
                                for (i3 = -1; i3 <= 1; ++i3)
                                {
                                    this.generateLeaves(world, k3 + l2, i2 + 1, l1 + i3);
                                }
                            }
                            ...
                            for (l2 = -2; l2 <= 2; ++l2)
                            {
                                for (i3 = -2; i3 <= 2; ++i3)
                                {
                                    ...
                                    {
                                        this.generateLeaves(world, k3 + l2, i2, l1 + i3);
                                    }
                                }
                            }
                        }
    ...
    private void generateLeaves(World world, int x, int y, int z)
    {
        ...
        {
            this.setBlockAndNotifyAdequately(world, x, y, z, ZenScape.blockZenLeaves, 1);
        }
        ...
        if (l == 0)
        {
        	...
        	if (block.isAir(world, x, y, z) && world.getBlock(x, y + 1, z) == ZenScape.blockZenLeaves && meta == 1)
        	{
        		this.setBlockAndNotifyAdequately(world, x, y - 1, z, ZenScape.blockFruitBomb, 0);
        	}
        }
    }
}
